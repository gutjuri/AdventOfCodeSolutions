...
parseField str = Field nm (min1, max1) (min2, max2)
 where
  (nm, _ : rest) = break (== ':') str
  [min1, max1, min2, max2] =
    map read $ words $ map (\c -> if isDigit c then c else ' ') rest
...
findInvalidValues fs = filter isInvalid
  where isInvalid x = not (any (fitsIn x) fs)

fitsIn :: Int -> Field -> Bool
fitsIn x (Field _ (min1, max1) (min2, max2)) =
  x >= min1 && x <= max1 || x >= min2 && x <= max2

fieldPossibilities :: Field -> [Ticket] -> [Int]
fieldPossibilities field ts =
  foldl' (\ps (i, v) -> if fitsIn v field then ps else delete i ps)
         [0 .. length (head ts) - 1]
    $ concatMap (zip [0 ..]) ts
...
sortOut assocs = (f, v) : sortOut (map (second (delete v)) $ delete u assocs)
  where u@(f, [v]) = fromJust $ find (\(_, x) -> length x == 1) assocs
...
p2 t fa =
  product $ map ((t !!) . snd) $ filter (isPrefixOf "departure" . name . fst) fa
...
day16 = do
  ...
  let validTickets =
        filter (null . findInvalidValues fd) (ownTicket : nearTickets)
