...
parseLn str
  | "mask" `isPrefixOf` str = let pl = drop 7 str 
                              in SetMask $ Mask (readBinary $ map (\x -> if x == 'X' then '0' else '1') pl ) (readBinary pl)
  | otherwise = let addr = read $ takeWhile (/=']') $ drop 4 str
...
allAddr m'@(Mask m1 m2) pos p 
  ...
  | otherwise = allAddr m' (pos `setBit` p) (p-1) ++ allAddr m' (pos `clearBit` p) (p-1)
...
day14 = do
  ...
  let sol1 = foldl' (+) 0 $ snd $ foldl' (\(m, im) c -> nextStep m c im) (Mask 0 0, M.empty) inp
  print sol1
  let sol2 = foldl' (+) 0 $ snd $ foldl' (\(m, im) c -> nextStepV2 m c im) (Mask 0 0, M.empty) inp
  print sol2