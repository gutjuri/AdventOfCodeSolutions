...
def splitSublistAware(line: String): List[String] =
    ...
    splitOn += line.length()
    val subs = splitOn.zip(splitOn.tail).map{ case (a, b) => line.substring(a+1, b)}.toList
...
def parseLine(ln: String): Data = 
    if ln.equals("") then DataList(List[Data]())
        ...
        then {
            val elems = splitSublistAware(ln.substring(1, ln.length()-1)).map(parseLine(_))
            DataList(elems)
        }   
...
def compare(left: Data, right: Data) : Int =
    (left, right) match
        ...
        case (DataList(li), DataList(lj)) => 
            li.zip(lj).map(compare).find(_ != 0).getOrElse(li.length.compareTo(lj.length))
...
@main def day13: Unit =
    val inp = parseInput
    val sol1 = inp.map(dp => compare(dp.left, dp.right)).zipWithIndex.filter(_(0) == -1).map(_(1) + 1).sum
    println(sol1)
    ...
    val sorted = inp.flatMap(p => Array[Data](p.left, p.right)).concat(Array[Data](divPacket1, divPacket2)).sorted(using (x, y) => compare(x, y))
