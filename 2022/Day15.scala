...
def cannotInLine(line: Int, info: (Point, Point)): Set[Int] =
  ...
  then Set.empty
    ...
    val (minX, maxX) = (
      sensor.x - (distToBeacon - distToLine),
      sensor.x + (distToBeacon - distToLine)
    )
...
def posCannotInLine(line: Int, info: Array[(Point, Point)]): Set[Int] =
  ...
  val pos = info
    .map(cannotInLine(line, _))
    .foldRight(Set.empty: Set[Int])((a, b) => a union b)
  pos diff beaconPos
  ...
  val boundary = {
    for (x <- 0.to(dist + 1))
      yield Array(
        ...
        p.x >= minCoord && p.x <= maxCoord && p.y >= minCoord && p.y <= maxCoord
      )
  }.flatten.toArray
  boundary

def getPossibleLocation(
    minCoord: Int,
    maxCoord: Int,
    info: Array[(Point, Point)]
): Point =
