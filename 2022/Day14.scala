...
def pourSand(
    minY: Int,
    cave: Map[(Int, Int), Char]
): (Map[(Int, Int), Char], Boolean) =
  var (sX, sY) = (500, 0)
  while (true)
...
def task2(cave: Map[(Int, Int), Char]): Int =
  ...
  var i = 0
  var caveX: Map[(Int, Int), Char] = cave ++ {
    for x <- (-floorLevel - 10).to(floorLevel + 10)
    yield ((x + 500, floorLevel), '#')
  }
